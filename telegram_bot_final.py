...
async def config_adult(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                "✅ Ja (+19,99€)" if config["adult"] else "Nein",
                callback_data="toggle_adult_no"
            ),
            InlineKeyboardButton(
                "✅ Ja (+19,99€)" if not config["adult"] else "Ja (+19,99€)",
...
async def checkout(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("💳 Mit Stripe zahlen", url="https://buy.stripe.com/7sY00lbe02cU2Vr9vWa7C00")],
...
async def pay_paypal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        "📧 Deine PayPal Email\n"
        "💰 Betrag\n\n"
        "Dann aktivieren wir deinen Account sofort! ⚡",
        reply_markup=reply_markup,
        parse_mode='Markdown'
    )
